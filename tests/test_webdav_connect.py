...
class TestWebdavConnect(unittest.TestCase):
    ...
    def test_send_request_token(self):
        """
        Tests that the `send_request` method returns a response when a token is provided.
        """
        # get token from config.env file
        self.connection.token = os.getenv('YADISK_TOKEN')
        response = self.connection.send_request("GET")
        self.assertIsInstance(response, requests.Response)

    def test_send_request_username_password(self):
        """
        Tests that the `send_request` method returns a response when a username and password are provided.
        """
    ...
    def test_send_request_no_credentials(self):
        """
        Tests that a `WebDavException` is raised when no credentials are provided.
        """
        self.connection.token = None
        self.connection.username = None
        self.connection.password = None
        self.assertRaises(WebDavException, self.connection.send_request, "GET")
